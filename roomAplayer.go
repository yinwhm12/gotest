...
func main()  {
	...
	m2 := make(chan int,1)
	m3 := make(chan int,1)
	player1 := syn.Player{PlayerId:1,RoomId: 1,}
	player1.RoomSignal = m
	player2 := syn.Player{PlayerId:2,RoomId: 1,}
	player2.RoomSignal = m1
	player3 := syn.Player{PlayerId:3,RoomId: 1,}
	player3.RoomSignal = m2
	player4 := syn.Player{PlayerId:4,RoomId: 1,}
	player4.RoomSignal = m3


	//Players[player1.PlayerId] = player1
	syn.Players[player1.PlayerId] = &player1
	syn.Players[player2.PlayerId] = &player1
	syn.Players[player3.PlayerId] = &player1
	syn.Players[player4.PlayerId] = &player1
